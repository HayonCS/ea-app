...
export const DashboardPage: React.FC<{}> = () => {
  document.title = "Dashboard | EA Performance";
  ...
  React.useEffect(() => {
    if (
      ...
      assets = assets.filter(
        (x) => date.getTime() - new Date(x.LastRunTime).getTime() <= 300000
      );
      assets = assets.filter(
        (value, index, self) =>
      ...
      assets = assets.filter(
        (x) =>
          comboAssetData.some((a) => a.Asset === x.Asset) ||
          processAssetData.some((a) => a.Asset === x.Asset)
      );
      setAssetsRunningNow(assets);
    }
  }, [runningNowQuery, comboAssetData, processAssetData]);
